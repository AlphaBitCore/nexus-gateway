...
-- The traffic drawer's pivot and the admin list's requestId filter both key on
-- external_request_id: it is the value the gateway hands back in the
-- X-Nexus-Request-Id response header, so it is the id an operator or a support
-- ticket actually holds, and it is what stitches the several rows one request
-- produces across services. Without this the filter is a sequential scan over
-- the whole retention window, run twice per page (COUNT + rows), on a table the
-- UI queries with no default time bound. Partial: rows recorded before any id
-- reached the record carry NULL, and there is no reason to index those.
CREATE INDEX IF NOT EXISTS idx_traffic_event_external_request_id ON public.traffic_event USING btree (external_request_id) WHERE (external_request_id IS NOT NULL);
-- trace_id is the CALLER's own W3C trace id, present only on requests that
-- arrived with a traceparent. It is indexed for the operator who starts from a
-- trace in their APM and wants the gateway's rows for it. Strongly partial by
-- nature: most callers send no traceparent, so most rows are NULL here.

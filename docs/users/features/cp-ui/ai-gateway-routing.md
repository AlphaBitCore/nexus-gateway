...
The adapter-type picker offers every PROVIDER wire format — the set `AllFormats()` returns — including the embeddings/rerank ones: `voyage` is configurable here like any other, even though it never serves chat. (`openai-responses` is deliberately not among them: it is an ingress format the gateway detects at the route layer, never a value you set on a provider.) A provider format the picker does not list has no way in at all, so the list is held to the gateway's own enum, the admin API's accept list and the published OpenAPI spec by `npm run check:adapter-type-lockstep` rather than by four comments asking to be kept in step.
...
A `smart` rule is the one strategy that will not save with empty match conditions: it needs at least one entry under **Requested model literals**, which is the list of `model` strings that hand the choice to the router. `auto` is the usual one because OpenAI-style clients already send it, but the words are yours — publish `fast` and `cheap` to your clients and list both, or list `gpt-4-*` to route a whole family. The wizard blocks two entries and only two: a blank one, which no request can ever send, and one made of nothing but `*`, which would quietly point every request in the fleet at the router. Everything else saves. Priority orders rules when more than one matches; the HIGHER priority runs first, and every other matching rule stays behind it as a backup for when it cannot serve the request. A strategy nested inside another is refused at save time — an entry names a provider and a model directly; to back a rule up with a different strategy, author that one at a lower priority.

**Routing preview.** The rule detail page answers "if a request like this arrived, which rule wins and what would it resolve to" without sending anything upstream. Two inputs: the model the caller would name (a catalog model id, or any keyword your rules delegate on — `auto`, or whatever your fleet publishes), and the ENDPOINT the request would arrive on. The preview distinguishes the two: a catalog model with no matching rule is served anyway, directly, because routing rules only redirect a named model; a keyword no rule claims has nothing behind it and is reported as a request that would be rejected.

For a `smart` rule the box is pre-filled with the rule's own first keyword, so Run simulation exercises the rule you are looking at rather than a string it does not match. A model id that names nothing in the catalog is treated as a delegating request and a sample prompt is sent with it — the router needs something to read, and without it the preview would show the rule's default model instead of the target it would really pick. No prompt is sent for a named catalog model, so a conditional rule's content clauses are not decided by text nobody typed. Suggestions are not offered for the rule's own keywords: they name no model, so every suggestion would be an offer to replace the keyword. The endpoint is the one that changes the answer most — a rule's model-type conditions, the modality filter and non-chat `auto` all key off it — so it is chosen rather than assumed; only the endpoints that actually constrain which models may serve them are offered, because the others accept any model and simulating them tells you nothing. When the preview cannot run, the gateway's own reason is shown rather than a generic failure, since this is the screen an admin is on precisely because routing is not doing what they expected.

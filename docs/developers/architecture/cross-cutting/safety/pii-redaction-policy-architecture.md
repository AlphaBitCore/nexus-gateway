...
The redacted bytes Nexus persists are the **raw wire copy** the adapter already rewrote in flight — not a separately recomputed normalized projection. The canonical normalized projection is **never persisted at all**; the control plane recomputes it at view time from the stored raw body, which is itself the action-governed (already-redacted under `redact` / `block`) copy, so the recompute reads PII-safe bytes. See [audit-pipeline-architecture.md](../observability/audit-pipeline-architecture.md) for the write/view split.
...
- **Block path** (`executeBlock`) — used when `onMatch.action = "block"` (also the default). Collects the redaction spans, and on a match stamps `Decision = RejectHard`, `Action = ActionBlock`, `ReasonCode = PII_DETECTED`, and the same `compliance:pii` + `severity:confidential` tags. The request is rejected, never forwarded. The audit row records the **policy attribution** — the matched `BlockingRule`, reason, and compliance tags — so an auditor can see what tripped the policy. A blocked request does **not** persist a content body: it never produces a masked wire copy (the spans only drive the reject decision), and the action-typed `StorageRawBody` withholds the raw bytes so no unredacted content is ever written.

- **Redact path** (`executeRedact`) — used when `onMatch.action = "redact"`. Walks the projection, collects per-pattern match offsets in the **original** segment text, applies replacements in descending start-offset order so successive rewrites do not shift earlier offsets, emits one `TransformSpan` per match, and stamps `Decision = Modify`, `Action = ActionRedact`, `ReasonCode = PII_REDACTED`. The same masked body is forwarded upstream, returned to the client, and persisted. If the target adapter cannot reverse-encode the masked content onto the wire (`ErrRewriteUnsupported`), the redact fails **closed** — the request/response is rejected rather than forwarded unredacted. Spans carry `Source = SourceHook`, `SourceID = pattern.id`, `Action = ActionRedact`, `ContentAddress` matching the projection slot (`messages.<i>.content.<j>` for chat text, `messages.<i>.content.<j>.toolResult` for tool-result output, `inputs.<i>` for `KindAIEmbedding` request payloads), and the resolved `Replacement` string.
...
The bytes Nexus persists are the **raw wire body** the adapter already produced in flight — there is no second, normalized copy redacted independently at audit time. The canonical normalized projection is **never persisted**; the control plane recomputes it at view time from the stored raw body (which is the action-governed copy), so the recompute never resurrects sensitive bytes. Nothing to redact independently means nothing that can drift out of step with the governed copy. See [audit-pipeline-architecture.md](../observability/audit-pipeline-architecture.md) for the write/view split and the view-time recompute.
...
The same exclusion binds the **hook-trace columns**. `request_hooks_pipeline` / `response_hooks_pipeline` persist a metadata-only projection of each `HookResult` — order, hook id/name, decision, reason, reason code, latency, tags, action, blocking rule — and deliberately omit `ModifiedContent` and `TransformSpans`. Both are in-flight working state that the rewrite paths consume while producing the wire body; nothing reads them back off the row. Serialising the whole struct would have put the message text (and, via `TransformSpan.Replacement`, the replaced text) into a JSONB column the storage gate never sees — once per hook, under `approve` as well as under `redact`. The AI Gateway's `appendHookTrace` and the Compliance Proxy / Agent's `hookTrace` both project onto this shape; neither marshals `HookResult` directly.

**Forensic fidelity when the raw body is dropped.** Because the normalized projection is recomputed at view time from the stored body, a row whose body was dropped (`NULL`) has nothing to recompute — a `block` whose raw body is not retained, or a `redact` whose adapter returned `ErrRewriteUnsupported`. Such a row still carries the decision metadata (rule IDs, category, reason code, hook trace) so an auditor sees *what* tripped the policy, but it has no reconstructable normalized view of the offending content — which is enforced, not incidental: the hook-trace projection above is what keeps the decision metadata free of the content it decided about. This is the deliberate safety-over-fidelity trade: never persist sensitive bytes. The one exception is an older shipped agent that still uploads its own governed normalized copy for a block/redact row — the Hub's retained MQ-consumer writer persists that copy as the sole forensic record for that row (see [audit-pipeline-architecture.md](../observability/audit-pipeline-architecture.md) §10.2).

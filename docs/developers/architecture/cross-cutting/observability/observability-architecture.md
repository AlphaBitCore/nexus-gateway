...
| Metrics | `packages/ai-gateway/internal/platform/metrics/` | (in-service `Recorder`) | none — the Agent has no business `Recorder` and no `/metrics`; it pushes `metrics_sample` over WS only | `packages/nexus-hub/internal/jobs/defs/rollup/` + `packages/nexus-hub/internal/jobs/defs/metrics/` | `packages/shared/core/metrics/registry/`, `instruments/`, `platform/` |
...
The canonical normalized representation produced by `shared/transport/normalize` has no table of its own — it is recomputed on demand (see below). The AI Gateway keys the normalize registry on the **ingress** wire format for both request and response (`audit.normalizeAdapterType` → `rec.IngressFormat`), never the routed upstream adapter — every body the gateway captures is already re-encoded to the client's ingress shape, so a non-Gemini model served over the Gemini ingress normalizes its response via the Gemini normalizer (`kind=ai-chat`) rather than falling through to `http-json`. See [normalization-architecture.md](../../services/ai-gateway/normalization-architecture.md) §5.1.
...
**Normalized projection (view-time).** The normalized projection is never persisted: the Control Plane recomputes it on demand from the stored, already-redacted raw body (`traffic_event` + `traffic_event_payload`) when the Traffic drawer opens, via `GET /api/admin/traffic/:id/normalized`. There is no sidecar table, no writer, and no normalize-backfill job. A row whose body is no longer recoverable — capture was off, or the spilled object aged out to retention — has nothing to project and the endpoint answers 404; that is an unavailable payload, not a gap to heal.
...
`traffic_event.endpoint_type` is the canonical `typology.EndpointKind` string (`chat`, `responses`, `embeddings`, `rerank`, `stt`, `tts`, `image_generation`, `video_generation`, `guardrail`, `batch`, `realtime`). The value is derived once per request — at AI Gateway handler-dispatch time via `string(typology.KindFromWireShape(resolved.WireShape))` for wire-shape-dispatched routes, or stamped directly by the handler for kinds that don't ride wire-shape dispatch (the `responses` label override, `guardrail`, and parallel-handler modality routes) — stamped onto `audit.Record.EndpointType`, and travels on the MQ wire as `TrafficEventMessage.EndpointType` (`packages/shared/transport/mq/messages.go`) into the Hub db-writer.
...
- **One vocabulary per concept.** `endpoint_type`, `cache_status`, `level` are canonical strings; consumers never translate. `error_code` is the deliberate exception, and the exception is per-producer rather than per-value: each producer writes one vocabulary, and a consumer reading the column sees the union. The AI Gateway writes upper-case codes for failures it decided itself (`PROVIDER_UNAVAILABLE`, `PROVIDER_RATE_LIMITED`, `QUOTA_EXCEEDED`, `CLIENT_CLOSED`) and the provider's own lower_snake cause for a terminal upstream 4xx (`auth_failed`, `invalid_request`, `context_overflow`, …) — the second set is carried through rather than re-cased precisely so it stays the same string the adapter classified, instead of becoming a second vocabulary for one concept. The Compliance Proxy pipeline writes its own. A consumer must therefore filter on the value it wants, never assume a case convention. See [error-taxonomy-architecture.md](../safety/error-taxonomy-architecture.md) §8.

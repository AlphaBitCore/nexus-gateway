...
**Normalized projection (view-time).** The normalized projection is no longer persisted on the write path: the Control Plane recomputes it on demand from the stored, already-redacted raw body (`traffic_event` + `traffic_event_payload`) when the Traffic drawer opens. The `traffic_event_normalized` sidecar is retained, write-frozen for new traffic, and the MQ consumer's `insertNormalizedPayloads` still persists a normalized copy only when a producer uploads one (an older agent build that still ships its governed projection) — its savepoint isolation means a sidecar failure rolls back only that savepoint while the raw bytes survive. There is no longer a normalize-backfill job: a missing sidecar is the normal state for current traffic and is filled by the view-time recompute, not a scheduled scan.

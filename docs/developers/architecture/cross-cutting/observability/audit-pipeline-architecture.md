...
- **View-time recompute (read side)**: the Control-Plane traffic detail drawer endpoint `GET /traffic/:id/normalized` recomputes the normalized payload **on the fly** from the captured (redaction-safe) inline bodies via the same `normalize.BuildRegistry()` chain, rather than reading the stored `traffic_event_normalized` sidecar — so the drawer always reflects the current `SchemaVersion` and the read path never depends on the sidecar being backfilled. It falls back to the stored sidecar only when no inline body is available (capture off / spilled). The stored sidecar + backfill remain for the list/search/analytics paths that query the column set.

...
The audit pipeline is the asynchronous fan-in path that turns every LLM traffic event observed by a data-plane service into a row on the Hub's `traffic_event` table (plus its `traffic_event_payload` sidecar). It is the substrate the Traffic drawer, cost dashboards, alerting engine, and the SIEM bridge all read from — a single canonical timeline of every request that touched the gateway, the compliance proxy, or an enrolled agent.
...
- `tools/db-migrate/schema/traffic.prisma` — `traffic_event` and `traffic_event_payload` models.
...
audit.Record (in-proc)
    │
    │ Enqueue (byte-budget admission: no-loss modes
    │ block on a full budget, lossy modes shed)
    ▼
bounded in-memory queue
(bytes ≤ ~15% RAM budget; count ≤ structural cap)
    │ N consumer workers (one per producer connection)
    │ batch of ≤512 (or 100ms linger) → marshal → frames
    ▼
producer batch publish ──► nexus.event.{ai-traffic|compliance|agent}
...
- **`Record.EventID` is the row's identity, and it is minted — never taken from the caller.** It becomes `traffic_event.id`, which is the key the Hub consumer's `INSERT … ON CONFLICT (id) DO NOTHING` uses to make a NATS redelivery idempotent, and it also keys the payload blob (`<date>/<eventID>-<direction>.bin`). `eventID()` in `packages/ai-gateway/internal/platform/audit/message.go` mints it once and stores it back on the record, because `marshalRecordPlain` deliberately rebuilds the message a second time when a marker collision knocks out the splice path, and a second id there would insert the very duplicate the conflict clause exists to prevent. Deriving it from the request id is what the mint replaces: that value comes from the caller's `X-Nexus-Request-Id` (or its `X-Request-Id` alias), which a client may legitimately reuse across retries, so a derived key collides two distinct events into one row and the conflict clause discards the second with no error to anyone. The request id keeps its correlation role on `external_request_id`, where repeating is by design — an agent flow's cross-service rows share one, and so do a realtime session's exchanges.
...
- On a publish failure the record is **re-queued for an in-memory retry up to 3 times** (`nexus_audit_mq_enqueue_errors_total` increments); past the cap — or when the queue is full — it is handed to the durable batched spill instead of circulating (the bounded-retry death-spiral fix). Never a silent loss.
- `Close()` stops intake, lets each consumer worker drain its remaining queued records and publish them (bounded at 512 per publish), drains the spill worker's channel, and durably spills any straggler that raced in after the final drain — a clean shutdown never leaves buffered audit unpublished or uncounted.
...
- `RequestNormalized` / `ResponseNormalized` exist on the wire as `json.RawMessage`, but nothing sets them and nothing persists them — the normalized projection is recomputed at view time from the stored raw body (§5), so these fields ride absent and `NormalizeVersion` stays unset on the audit path. They are numbered fields in a binary inter-service format (`shared/transport/mq/binwire.go`), so they remain allocated: removing them is a gateway↔Hub protocol change, and the Hub simply stops persisting what it still receives.
...
- **Vendor-spend attribution (`RouterCostUsd` / `RouterProviderID` / `EmbeddingProviderID`, 2026-08-04).** `recordToMessage` carries three fields the Hub rollup needs to attribute internal-ops spend to the vendor that was actually charged for it, since that vendor is frequently **not** the one that served the request (production measured ~38% of smart-router calls landing on a different vendor than eventually served the request). `RouterCostUsd` is the smart-router LLM call's own cost (summed across every router call the request's routing trace recorded); `RouterProviderID` is the provider that **served** that call (`decision.ServedProviderID`, never `decision.ProviderID` — the vendor of the model the router *picked*), first-write-wins if more than one router call ran. `EmbeddingProviderID` is the sibling attribution for the existing `EmbeddingCostUsd` / `EmbeddingModelID` pair — the provider that served the L2 semantic-cache embedding call (see `cache-multi-tier-architecture.md`). All three are zero/empty when the corresponding call did not run or failed — a failed router call is not billed. New binwire field ids `109` / `110` / `111` carry them; the wire is forward-incompatible (an old Hub silently drops an unknown field id), so the deploy order is **schema → Hub → producers**. See [cost-estimation-architecture.md](../../services/ai-gateway/cost-estimation-architecture.md) for the cost-stamping detail and [metrics-rollup-architecture.md](metrics-rollup-architecture.md) for how the Hub rollup turns these into the `vendor_spend_usd` / `vendor_spend_internal_usd` series.
...
2. **On-disk** JSON-Lines DLQ — used **only** when (1) fails, i.e. the DB is unreachable. This is the durability guarantee for a full DB outage: the raw billing/audit bytes are appended to `<TMPDIR>/nexus-hub-dlq/traffic-event-dlq.jsonl` (one JSON object per line: `msgId`, `subject`, `payload` (base64), `deliveryCount`, `lastError`, `writtenAt`) so they are never silently dropped at the redelivery cap. **Replay**: once the DB recovers, an operator imports each line into `traffic_event_dlq` (the record shape maps 1:1 to the table columns) and uses the existing `POST /api/hub/dlq/:id/retry` to republish; the file is then rotated/truncated. Counter: `nexus_mq_disk_dlq_inserted_total{subject}`.
   ...
   **Size cap.** The file is bounded at **256 MiB** (`defaultDiskDLQMaxBytes`). It had no bound at all: append-only, holding raw MQ payloads — request/response bodies, `source_ip`, `end_user_id` — with nothing truncating or rotating it, so a sustained DB outage grew it until the host ran out of disk, and every byte written stayed for the life of the host.

   Reaching the cap **refuses the append rather than rotating**, and that choice is the one this sink exists to make. An append error means the caller keeps the message on the BROKER, which is itself durable and bounded by the broker's own retention — so the failure mode is back-pressure, not loss. Rotating would silently discard the OLDEST dead letters, which is precisely the outcome the on-disk DLQ exists to prevent. The size is adopted from the file at open (one `Stat`, not one per append), so a Hub restart does not reset the cap and let an already-full file keep growing.
...
The normalized projection is **never persisted**. No producer (ai-gateway, compliance-proxy, agent) stamps it, the Hub does not derive or backfill it, and there is no table holding it. The raw rows commit on `traffic_event` + `traffic_event_payload` and the projection is computed from them on demand.

Erasure is the reason this matters beyond tidiness: with one stored copy of the captured text instead of two, a GDPR Art.17 scrub has one surface to clear. A second stored copy is a second thing an erasure can miss.

- **Read side (the source of truth)**: the Control-Plane traffic detail drawer endpoint `GET /traffic/:id/normalized` recomputes the normalized payload **on the fly** from the captured (redaction-safe) inline bodies via the `normalize.BuildRegistry()` chain — so the drawer always reflects the current `normcore.SchemaVersion` with no migration or scheduled job. A spilled body is fetched back from the spill store and recomputed the same way. When neither is recoverable — capture was off, or the spilled object aged out to retention — there is nothing to project and the endpoint answers `404`.
...
| `nexus_mq_traffic_errors_total` | `error_type` (`deserialize` \| `db_begin` \| `db_insert` \| `db_insert_payload` \| `db_commit` \| `dlq_insert` \| `disk_dlq`) | Per-failure-class breakdown. `dlq_insert` / `disk_dlq` = a DB-backed / on-disk dead-letter write failed |
...
- `tools/db-migrate/schema/traffic.prisma` — `traffic_event`, `traffic_event_payload`

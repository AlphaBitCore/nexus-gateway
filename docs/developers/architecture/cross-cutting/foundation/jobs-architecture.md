...
|---|---|---|---|
| `rollup-5m` | `defs/rollup/rollup_5m.go` | 1 min | Aggregates `traffic_event` rows into `metric_rollup_5m` every minute, catching up from the last committed bucket to the most recent sealed 5-minute bucket. Also emits the `traffic.error_class.*` series (failed rows keyed by errorCode\|statusRange\|provider\|model, 500-classes-per-bucket cap with `__overflow__` fold) that backs the Control Plane error-governance view's rollup-first read, and the `vendor_spend_usd` / `vendor_spend_internal_usd` series (`rollup_5m_vendor_spend.go`) that back vendor-bill reconciliation — emitted per cost component under the dimension of the provider actually charged, so one row can land on several `routed_provider` dimensions. |
...
| `rollup-correction` | `defs/rollup/rollup_correction.go` | 24 hour | Recomputes fleet rollups across the trailing correction window (default 7 days, clamped to `Rollup5mDays - 1` so it never contends with `rollup-retention` for the same 5m buckets — the overlap deadlocked the 2026-08-07 run and stalled the cursor) to absorb late-arriving events. Re-runs every 5-minute bucket then re-merges the 1h, 1d, and (for any sealed month the window touched) 1mo layers, with the per-bucket watermark write suppressed so the backfill never rewinds the live cursor. On success it publishes its own `rollup-correction` watermark — the newest UTC day it rebuilt (yesterday) — which `vendor-bill-reconcile` reads to tell an absent series apart from one not yet re-aggregated; a run that failed mid-window leaves the previous value standing. Because it drives the same per-bucket aggregation as `rollup-5m`, it also (back)fills the `traffic.error_class.*` series — including, once after the series first ships, the trailing week of history (triggering it right after that deploy shortens the error-governance view's rollup warm-up). |
...
| `vendor-bill-reconcile` | `defs/vendorbill/reconcile.go` | 24 hour | Pulls each covered provider's authoritative daily billed USD from the vendor cost API (OpenAI/Anthropic via `internal/vendorbill` sources) over a trailing window (ends today−2, N=8 days — wide enough that a day cannot age out of it while `rollup-correction` is stalled, and wide enough that one pass after a deploy backfills the preceding week), diffs against our recorded vendor spend (`metric_rollup_1d` `vendor_spend_usd`, `routed_provider` dim — internal ops included), and UPSERTs per-provider-per-day rows into `vendor_bill_reconciliation` (`our_vendor_spend_usd` = the diff basis, `our_internal_ops_usd` = its internal-ops subset, `our_billed_usd` = the unchanged customer-quota figure). `coverage` = `scoped` \| `org_only` \| `fetch_failed` \| `no_basis`; a fetch failure records no vendor number and no diff (never fabricated); a day the vendor reported a bucket for but charged nothing on, with no recorded spend either, is UPSERTed as a real `scoped` zero row rather than skipped — an absent row and a zero row are the same blank in the report, and the zero row is also the only thing that can heal a `fetch_failed` placeholder stranded on an idle day (the vendor has no non-zero figure for such a day, ever, so prod OpenAI 2026-08-16 sat frozen at `fetch_failed` raising `vendor.bill_sync_failed`); only a day the vendor omits from its response entirely is skipped as unfinalized; a day past the `rollup-correction` watermark is deferred entirely — neither compared nor placeholdered — because its `vendor_spend_usd` rows may still be unwritten or partial, and both misreadings are damaging (a fabricated `no_basis` ages into `vendor.bill_sync_failed`, a partial basis fires `vendor.bill_drift`). Since both jobs fire on the same tick and the cursor is only published when the ~100s correction pass ends, the run first waits up to 10 min (polling every 15s) for the cursor to reach its window end, and **fails** if the cursor has not reached even the window start — a run that can reconcile nothing has done none of its work, and returning success there hid three consecutive empty runs on stg (2026-08-06..08-08) behind a green job status; a day the correction pass HAS rebuilt that still has no `vendor_spend_usd` rows is not compared (absence ≠ zero — pre-rollout rows keep their old-basis values untouched) but gets a display-only `no_basis` placeholder via `ON CONFLICT DO NOTHING` carrying the real vendor number, so a broken cost-stamping path surfaces in the report and ages into `vendor.bill_sync_failed` instead of vanishing. A 5xx or transport failure is retried on an exponential backoff (0.5s doubling, 4 attempts); a 429 instead backs off past the vendor's rate-limit window (65s, then up to 90s), because OpenAI meters its org-wide 30/min admin limit in fixed one-minute windows and the exponential schedule spent all four attempts inside one — which is how a single 429 cost the provider its whole window on prod (2026-08-16, 2026-08-18). `Retry-After` is honoured and preferred over both. Providers are resolved by `adapter_type` **and** matched on `baseUrl` host, so an OpenAI-compatible self-hosted model is not handed the real vendor's bill as its own. Admin keys are env-only. The two diff figures use different bases on purpose: `diff_usd` is computed from the cent-rounded pair (the vendor bill is only meaningful to the cent), while `diff_pct` reads the raw pair — a ratio taken from rounded operands carries the rounding as error scaled by `1/vendor`, which pins low-volume days at exactly `0.0%` and hides the drift there. See the "diff_usd is rounded to the cent; diff_pct is not" section of [vendor-bill-reconciliation.md](../../../../operators/ops/runbooks/vendor-bill-reconciliation.md). |
...
| `provider-health-rollup` | `defs/rollup/provider_health_rollup.go` | 5 min | Recomputes `ProviderHealth` (error rate, avg latency, sample count, status) from `traffic_event` over a 30-minute rolling window. Replaces the AI Gateway in-process HealthTracker DB flush. An adverse verdict — `degraded` or `unavailable` — needs at least 20 requests in the window: a rate over a handful of requests is not a rate, and without the floor one failed request put a provider at 100% error, fired `provider.unavailable`, and could not be cleared by hand because the status is derived. `sampleCount` is written whatever the verdict, so a reader can see what it rests on. |
...
| `vendor-bill-sync-alerts` | `defs/vendorbill/sync_alert.go` | 24 hour | Class-1 state-poll: raises `vendor.bill_sync_failed` (per provider) when a `fetch_failed` **or** `no_basis` row stays unhealed past `staleHours` (default 25 — one run cycle + margin) — i.e. either the vendor cost-API sync is persistently broken (revoked/wrong-type key, lost scope, blocked egress) or our own cost stamping recorded no vendor spend for a day the vendor billed. Uses `updated_at` age as the "didn't self-heal" signal so a transient never ages into an alert; placeholder inserts are `DO NOTHING` so they cannot reset that clock. Auto-resolves on recovery. Closes the blind spot the drift job leaves by treating both coverages as display-only. |

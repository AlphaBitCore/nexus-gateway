...
### 5.1 Traffic + ops rollup pipeline (12 jobs)
...
| `override-expiry` | `defs/expiry/override_expiry.go` | (cfg, RunOnStart) | Clears `thing_config_override` rows past their `expires_at`. |
| `diag-mode-expiry` | `defs/expiry/diag_mode_expiry.go` | 1 min | Clears the `diagModeUntil` flag from `thing.metadata` once the corresponding `thing_diag_mode_window` has ended. Mutations are atomic per Thing (no batch transaction). |

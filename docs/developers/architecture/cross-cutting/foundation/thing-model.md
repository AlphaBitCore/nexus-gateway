...
**Auth and connection** (`schema/nodes.prisma`): `auth_type` ∈ {`bearer`, `mtls`, `apikey`} default `bearer`; `conn_protocol` ∈ {`http`, `websocket`} default `http`. (The schema string also lists `mqtt`, but no live code path produces it — confirmed by `grep -rE 'mqtt|MQTT' packages/ --include='*.go'` returning zero non-test hits.)
...
**Static identity (`metadata.staticInfo`)**: beyond the promoted columns, every Thing pushes a `static_info` payload (captured by `packages/shared/core/metrics/platform/staticinfo.go` `CaptureStaticInfo`, typed as `registry.StaticInfo` in `packages/shared/core/metrics/registry/types.go`) that Hub stores under `thing.metadata.staticInfo`. Two of its fields are the Thing's self-reported base URLs:

- `publicUrl` — the **externally reachable** base URL clients use to reach the service: external callers **and the agent** (the agent runs on end-user devices and is an external caller by definition). Populated from the service's own `publicURL` yaml/env config.
- `privateUrl` — the **internal service-to-service** base URL peer Nexus services dial. Populated via `platform.EffectivePrivateURL`: the service's own optional `privateURL` yaml / `<SVC>_PRIVATE_URL` env override when set, else auto-derived from the BIND interface: a service bound to a specific address (the single-box appliance binds `127.0.0.1` behind nginx) advertises that address — a loopback-bound socket refuses dials to the machine's LAN IP, so loopback binds must advertise loopback; a wildcard/empty bind advertises `http://<primary-outbound-IPv4>:<service-port>`. **Empty for agent Things** — the agent is a client, never a service-to-service call target, and the private URL is never pushed toward end-user devices.

A service only ever reports its OWN two addresses; peers resolve the reported values from the Hub instead of configuring each other's URLs (see `service-call-framework.md` §6.5).

Three more of its fields answer **which build a node is running** — `serviceVersion`, `buildSha`, `buildTime`. `BuildInfo` takes the two raw inputs a service actually has, its own name and the `-ldflags` stamp in `main.buildVersion`, and `CaptureStaticInfo` resolves all three via `platform.ResolveBuildIdentity`. Precedence is two levels, not three: the sha carried by a `tag@sha` build version (the release build already stamps `-X main.buildVersion=prod-<date>@<sha>`), else the VCS stamp Go embeds for builds made inside a git checkout. There is no separate "explicit `-ldflags` value" level — the `-ldflags` stamp IS the build version, and the first level reads it. The sha is taken only when what follows the `@` is shaped like one (7-40 lowercase hex, git's own abbreviation floor); anything else falls through to the VCS stamp, because a recipe stamping a word there used to both fabricate a sha and suppress the real revision sitting beside it. A `+marker` suffix survives — `+vs` records that the binary linked vectorscan rather than falling back to RE2. A tree with uncommitted changes is marked `+dirty` on BOTH levels, including the tagged one that is the only level producing a sha in prod; an unqualified sha for a build that is not that sha sends a reader to source that is not what is running.

The three fields are resolved inside `CaptureStaticInfo` rather than filled in by each caller, because when callers filled them in, every caller got them wrong: three of the five services reported a hardcoded `"<service>/0.1.0"` and all five reported an empty `buildSha`, so no environment could be tied to a build at all and "is this fix deployed?" had to be answered by replaying traffic. Taking the raw inputs removes the opportunity to get it wrong instead of adding a check that watches for it.

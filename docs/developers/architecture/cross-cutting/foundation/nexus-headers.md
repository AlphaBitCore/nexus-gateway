...
- **One request id, two accepted spellings.** `X-Nexus-Request-Id` is the canonical name and `X-Request-Id` is its compatibility alias — the same concept, not two ids. A caller that already stamps the industry-conventional `X-Request-Id` on its outbound traffic is understood without changing a line; a caller that wants the unambiguous Nexus name sends `X-Nexus-Request-Id`. The gateway reads the canonical name first, falls back to the alias, and generates a UUID when neither arrives. The resolved value is echoed on the response and persisted to `traffic_event.external_request_id`, which is what a support ticket quotes.

  It also stitches the several rows one request produces as it crosses services — **when the caller sent an id**. Each service resolves the same value from the same request and records it, so the rows join. A caller who sends none gets an independently minted id per service, because no service forwards `X-Nexus-*` onward (§8.1): those rows are correlated by client IP and time window instead, by the Hub's identity enricher. Sending an id is therefore the difference between one query returning a request's whole path and returning one hop of it.
- **There is no `X-Nexus-Trace-Id`, and there will not be one.** Distributed tracing is a solved problem with a standard header: callers send W3C **`traceparent`**, Nexus continues that trace through its own spans, and persists the caller's trace id to `traffic_event.trace_id` so a row in the traffic table joins back to the caller's APM. Minting a proprietary `X-Nexus-Trace-Id` / `-Flow-Id` / `-Correlation-Id` alongside it would be a second, worse spelling of something every tracing vendor already speaks — and drop-in compatibility is the point of this gateway.
...
| `X-Nexus-Request-Id` | AI Gateway middleware (`packages/ai-gateway/internal/platform/middleware/middleware.go`); echoed by CP and Agent via the shared tlsbump `stampMarkers` (`packages/shared/transport/tlsbump/markerhook.go`) | UUID string | The request id, echoed back under its canonical name whichever spelling the caller used (§1). AI Gateway resolves it from `X-Nexus-Request-Id`, then `X-Request-Id`, and generates one when neither arrived; CP and Agent reflect the incoming value onto the response, generating one when absent. Same header on request and response, and the value persisted to `traffic_event.external_request_id`. |
...
| `X-Nexus-Attempts` | every response that reached routing | integer-as-string (≥1) | Number of routing attempts the gateway made. `1` = first-try success; `2+` = at least one L2 retry or L3 failover. Emitted with no `> 0` gate, so an observer on a routed response always sees the value — but a request refused before routing (admission 503, auth 401, payload 413, a hook-stage 403) carries no attempts because none were made. |
| `X-Nexus-Coerced` | the codec, when it rewrote a field to satisfy the upstream wire | comma-separated `field→reason` descriptors (e.g. `max_tokens→4096_model_max`) | What the provider codec had to change to make the request legal for the target — a value clamped to a model ceiling, a field renamed or dropped. Produced by the codec, not by the hook pipeline. Empty when nothing was rewritten. |
...
| `X-Nexus-Request-Id` | AI Gateway middleware; every service | The request id under its canonical name — honoured if present, echoed on the response (§1, §2.1), threaded across every Nexus hop, persisted to `traffic_event.external_request_id`. When absent the gateway falls back to `X-Request-Id`, and generates a UUID only when neither arrived. Quote this value in a support ticket. | Shipped |
| `traceparent` | `telemetry.HTTPTrace` via the registered W3C TraceContext propagator | The caller's distributed trace. Nexus continues the trace rather than starting its own, and persists the caller's 32-hex trace id to `traffic_event.trace_id` so a traffic row joins back to the caller's APM (Datadog, Honeycomb, Jaeger, any OTLP backend). Persisted **only** when the inbound header parsed: a caller who sends nothing gets `trace_id` NULL, never a Nexus-invented value, because a column that means "the caller's trace" must be empty when there is no caller trace to point at. Vendor spelling — a W3C standard name, not part of the `X-Nexus-*` namespace. | Shipped |
| `X-Request-Id` | proxy pipeline setup | **Compatibility alias for `X-Nexus-Request-Id`** — the same request id under the industry-conventional spelling, so a caller whose stack already stamps it is understood without changing a line. Deliberately carries no `X-Nexus-` prefix for exactly that reason. Read only when the canonical name is absent, then treated identically: echoed, threaded across hops, persisted to `traffic_event.external_request_id`. Never rewritten. Recorded on every producer path, not just the AI Gateway's: tlsbump reads it off the intercepted request and hands it to the audit emitter on `AuditInfo`, so a caller whose traffic reaches Nexus through the compliance proxy can join on it too. On the RESPONSE the header carries exactly one value: the caller's when they sent one, the upstream's otherwise. The `RequestID` middleware echoes the caller's, and `writeForwardedResponseHeaders` skips any header the gateway has already stamped rather than appending — emitting both left two values under one name, and every client library's `Get` returns the first, so the caller read back their own id and the provider's was invisible. | Shipped |
| `X-Nexus-End-User-Id` | record construction (`stampCallerAttribution`) | The caller's own end-user tag, persisted to `traffic_event.end_user_id` so an external system can join gateway traffic to its user table per user. The header is the only source, on every ingress shape. A provider's own end-user field (the OpenAI shape's top-level `user` or `safety_identifier`, the Anthropic shape's `metadata.user_id`) identifies the caller's end user to that provider and answers a different question, so it is not read here. Opaque and caller-asserted: never validated, never resolved against Nexus identities, never used for quota / routing / IAM — meaningful only within the caller's VK scope. Trimmed, capped at 256 bytes. **Stored verbatim and NOT redacted**: the compliance pipeline governs captured bodies, not this promoted column, so a caller that puts PII (an email) in its end-user ids is storing that PII queryably — use opaque ids. All three tags (end-user, session, client tags) are stamped when the audit record is BUILT, which is before admission runs auth, rate limiting and the body read — so a row rejected at any of those still carries them. A missing value therefore means the caller sent no header, not that the request failed early. Client tags have no column of their own: absent tags leave `details` with no `clientTags` key at all, never a literal `null`. | Shipped |
| `X-Nexus-Session-Id` | record construction (`stampCallerAttribution`) | The caller's session/conversation tag — the grain between the end-user tag and `X-Request-Id`, persisted to `traffic_event.session_id` so an external system can group one conversation's requests (cost per thread, dialogue replay). Header-only: chat protocols carry no reliable native session field. Same trust model as the end-user tag; trimmed, capped at 256 bytes. Stamped where the record is BUILT, not later in admission: six functions in the proxy package construct an `audit.Record` and only the main proxy path extracted these tags, so the end-user, session and client-tag headers worked on `/v1/chat/completions` and were dropped on realtime, transcription, guardrail and both video arms. `stampCallerAttribution` is the one call all six make, and a `go/ast` test over the package fails when a seventh appears without it. **Also read by smart routing** for prompt-cache session affinity (`smart-routing-architecture.md` §4a): a chat request carrying this tag keeps its conversation on the model whose prompt cache is already warm, provided that model is still in the pool the router chose from. That is the one place a caller-asserted tag influences routing, and it can only reorder candidates the request already qualified for — it never relaxes a filter, and the key is scoped by virtual key so one caller's tag cannot reach another's traffic. A request without the header does not participate at all. | Shipped |
| `X-Nexus-Client-Tags` | record construction (`stampCallerAttribution`) | A comma-separated bag of client-asserted tags, formatted as `key=value` pairs, persisted to `traffic_event.details.clientTags` so an external system can attribute traffic within the caller's domain. Keys must match `^[a-z0-9_]{1,32}$`; an invalid key is dropped individually. Only the first `=` in a segment separates key from value, so a value may itself contain `=`; a comma has no escape mechanism, so one embedded in a value doesn't just split the segment — it can *manufacture* an extra tag (`note=a,b=c` yields `note=a` **plus** a real `b=c`, and the phantom `b` key consumes one of the 8 slots). Values are trimmed and capped at 256 bytes without splitting a multi-byte character. At most 8 distinct keys per request; a new key beyond the cap is dropped (repeating an existing key still overwrites it). A segment with no `=` is dropped; duplicate keys, last wins. Caller-asserted, never validated, never resolved against Nexus identities, never used for routing / quota / hooks / caching / IAM / metrics. **Stored verbatim and NOT redacted, across up to 8 keys**: the compliance pipeline governs captured bodies, not `details`, so PII placed in a tag value is stored queryably. `details` is forwarded whole to the SIEM export and rendered in the control-plane traffic drawer, and it is **not cleared by a GDPR Article 17 erasure** (the DSAR erasure flow nulls `entity_id` / `entity_name` / `identity` / `source_ip` / `source_process` but never touches `details`) — use opaque ids, never personal data. Absent when no valid pair survives. | Shipped |
...
**The `X-Nexus-*` namespace stops at the last Nexus hop, with one exception.** The AI Gateway denies the prefix on its forward path, and the interception path — the compliance proxy and the agent — strips it in `UpstreamTransport.ForwardRequest`. Both directions of that rule matter. We never *add* one: an intercepted request was addressed to the provider, not to Nexus, so a header we attach announces Nexus to a third party the caller never told about us and hands them a correlation id of ours. And we never *relay* one the client sent: a client speaking the Nexus vocabulary is addressing Nexus, and the header stops where it was addressed. The correlation id still exists on every hop's audit row; it simply does not travel on the wire.

`X-Request-Id` is untouched by this rule — it carries no Nexus prefix precisely because it is the caller's own header under the industry-conventional name, and providers read and echo it.

**The exception is `X-Nexus-Attestation`.** The agent stamps it through the per-request injector, which runs AFTER the strip (`upstream.go`), so it survives — deliberately. It has to: the compliance proxy is the intended reader, and the agent cannot tell from a bumped flow's target host whether the next hop is the compliance proxy or the provider itself. When the agent egresses directly, the attestation goes to the provider. That is an accepted cost rather than an oversight: the header carries a signature over request metadata and no credential, so a third party learns that some proxy signed the call and nothing it can use. `TestForwardRequest_InjectorRunsAfterTheStrip` pins the ordering, because reversing it would silently stop the agent attesting and the compliance proxy would quietly fall back to treating every agent as unattested.

**The correlation ids, by the question each answers.** Five of the roster's headers exist only to make traffic findable later, at five different grains. Sending none of them is fine — the gateway still records every request — but each one you send buys a query you could not otherwise run.

| I want to find… | Send | Lands on | Grain |
|---|---|---|---|
| this one request, from a support ticket or my own logs | `X-Nexus-Request-Id` (or `X-Request-Id`) | `external_request_id` | one request; also stitches the several rows one request produces across services |
| every request one of my end users made | `X-Nexus-End-User-Id` | `end_user_id` | one user, many sessions |
| every request in one conversation | `X-Nexus-Session-Id` | `session_id` | one conversation, many requests |
| this traffic from inside my own APM trace | `traceparent` | `trace_id` | whatever your tracer scoped the trace to |
| traffic by a fact only my system knows | `X-Nexus-Client-Tags` | `details.clientTags` | whatever you tagged |

The first four are exact-match filters in the Control Plane traffic list; the fifth is carried to the SIEM export and rendered in the traffic drawer. The three tag headers (`End-User-Id`, `Session-Id`, `Client-Tags`) are caller-asserted and never resolved against Nexus identities — see the trust boundary below.

**Caller-asserted tags versus Nexus attribution.** `end_user_id`, `session_id` and `details.clientTags` answer *"who, in the caller's own world"*: their namespace belongs to whoever sent the traffic, Nexus stores the value verbatim and never resolves it against anything. `entity_type` / `entity_id` / `entity_name` / `org_id` / `identity` answer *"who, in Nexus's world"* — a Nexus user, project or org, resolved server-side and never copied from a caller-supplied field. These are two orthogonal axes, not two spellings of one, and the test for which column a value belongs in is **whose word it is**, not who wrote the row: a conversation id the compliance proxy lifts out of intercepted traffic was written by Nexus but is still the intercepted client's assertion, so it belongs on the caller-asserted axis. Nexus resolves the attribution axis three ways depending on the service — the AI Gateway from the virtual key, the Agent from its signed-in session, the Compliance Proxy after the fact — and the confidence difference is already carried by `identity.status` (`matched` when resolved at request time, `pending` awaiting enrichment, `ambiguous` when the evidence fit more than one subject).

**When a VK carrier cannot be resolved**, the answer distinguishes whose fault it is. A key that is absent, malformed, unknown, disabled or expired is the caller's problem and answers `401`. A key that could not be *looked up* — the datastore is unreachable, the query failed — is ours, and answers `503`: `vkauth` reports it as `ErrUnavailable` rather than folding it into `ErrInvalid`. The distinction is not cosmetic. Under the old behaviour a database outage told every caller in flight that their virtual key was invalid, so they rotated keys, which cannot help, while the real fault stayed invisible in the one signal operators watch.

...
**The fingerprint records an outcome, not an attempt.** The lifecycle records a snapshot's fingerprint BEFORE calling `EnsureIndex`, which is what deduplicates concurrent snapshots — and that same record is what makes a later snapshot carrying the same fingerprint take the "unchanged; skipping" early return. So a failed `EnsureIndex` must UN-RECORD it. Without the rollback, one transient failure — a Redis blip, a momentarily-unavailable search module — marked the fingerprint handled forever: every subsequent snapshot skipped, the index was never created, and the semantic cache stayed dead for the lifetime of the process while the shadow callback reported nothing worse than a WARN.

The rollback is conditional on nothing having advanced past it — it restores the previous value only while the recorded fingerprint and index name are still the ones this call wrote. Otherwise an older call's failure would clobber a newer snapshot's successful record. Errors still do not propagate to the caller; the shadow callback must not block.

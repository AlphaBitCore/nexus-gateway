...
The request `model` field is a customer-facing string (`Model.code` such as `gpt-4o`, or a request-side sentinel such as `auto`), not a UUID. `Resolver.hydrateRequestedModel` resolves it through `ResolveModelCandidates`, which returns every enabled `Model` row whose `code` matches exactly or whose `aliases` list contains the string.

`ResolveModelCandidates` distinguishes **an unbuilt catalogue from a code the catalogue does not carry**, and the distinction is load-bearing rather than defensive. The cache-layer implementation returns `cachelayer.ErrIndexUnavailable` while the models snapshot has never loaded, which sets `RequestedModel.HydrationFailed` and makes provider-scoped conditions REFUSE rather than widen. Answering that state with an empty candidate list instead reads downstream as "the caller named nothing", which silently widens every provider-scoped rule to every provider and reports the model as absent from the catalogue — a permanent verdict on a condition that clears on its own. The loaded signal is the `modelsByCode` index, not `models.All()`: `configcache.NewSnapshotCache` pre-seeds an empty map, so `All()` is non-nil from construction and cannot tell "never loaded" from "loaded and empty". The matching `Model.id` UUIDs are recorded on `RequestedModel.CandidateIDs`, and the provider, type, and provider-model id are filled from the first candidate when empty.

The `auto` sentinel is intentionally left without candidates: a rule cannot accidentally route an `auto` request through a UUID-keyed `matchConditions.models`. Such requests must be authored against `matchConditions.requestedModelLiterals`, which matches the raw request string. Any other routing keyword lands in the same place without a special case — it names no `Model.code`, so `ResolveModelCandidates` returns nothing and `CandidateIDs` stays empty, which is exactly the state `auto` is short-circuited into. That emptiness is the load-bearing fact downstream: `callerNamedTheModel`, the requested-side audit columns, and the simulate preview's warnings all read it rather than comparing against the string `auto`.
...
- `requestedModelLiterals` — raw request strings (such as `auto`) that are not `Model.code` values, compared with `matcher.MatchGlob`, so `gpt-4-*` is a legal entry and is the only way to write a rule that survives the next version-suffixed release. A `smart` rule must carry at least one, and the admin API refuses a blank entry or an all-`*` entry; every other keyword, in any number, is the deployment's to choose.

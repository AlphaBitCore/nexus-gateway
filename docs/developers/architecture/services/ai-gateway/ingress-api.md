...
The request's `model` field drives routing (see [routing-architecture.md](routing-architecture.md)). Send a concrete model and the gateway resolves it to a provider+model target through the active routing rules; send a delegating keyword — `auto` by OpenAI-side convention, or whatever the deployment's smart rules pin under `matchConditions.requestedModelLiterals` — to hand model selection to the smart router (see [smart-routing-architecture.md](smart-routing-architecture.md)).

**The string is trimmed, and a blank one is refused.** `ExtractIngressModel` applies `strings.TrimSpace` once at its single exit, so every ingress shape — body `model`, the Gemini path segment, the Azure deployment name — is normalized identically, and `{"model": "  gpt-4o  "}` resolves to `gpt-4o`. A model that is whitespace-only trims to empty and is answered `MODEL_REQUIRED`, the same as omitting the field: the caller named no model. Untrimmed, the string is compared byte-for-byte in three places that each fail differently — the catalogue lookup reports a model that exists as absent, a `requestedModelLiterals` glob misses the rule written for the request, and `traffic_event.model_name` splits one model across two analytics keys. The same normalization applies to the `GET /v1/models/{model}` retrieve route, which is the other client entrance for a model string.
...
To find one request again afterwards, send `X-Nexus-Request-Id` — or
`X-Request-Id`, which is the same header under the spelling most stacks
already emit; the canonical name wins if both arrive, and the gateway mints a
UUID when neither does. The resolved value comes back on the response, lands
on `traffic_event.external_request_id`, and is the id that stitches together
the several rows one request produces as it crosses the agent, the compliance
proxy and the gateway. It is what a support ticket should quote.

If your stack is instrumented with OpenTelemetry it already sends
`traceparent`. The gateway continues that trace rather than starting its own,
and records the caller's trace id on `traffic_event.trace_id`, so a span in
your APM leads to the gateway's row for the same call. That column is written
only when an inbound `traceparent` parsed — with no caller trace it stays
NULL rather than receiving an id the gateway invented.
...
bytes, and neither affects quota or IAM.

`X-Nexus-Session-Id` has one further effect, and it is the only case where a
caller-asserted tag reaches routing: smart routing uses it to keep one
conversation on the model whose prompt cache is already warm, which is worth
real money because a provider's cache is keyed on model plus prefix. The tag
can only reorder candidates the request already qualified for — see
[smart-routing-architecture.md](smart-routing-architecture.md) §4a — so it
never relaxes a filter, and a request that does not carry it routes exactly as
before.

For more structured attributes, use `X-Nexus-Client-Tags` to send a
comma-separated bag of `key=value` pairs (e.g.
`X-Nexus-Client-Tags: tenant=abc,region=us-west,env=prod`). Keys must match
`[a-z0-9_]` and be 1–32 characters; invalid keys are silently dropped. A segment
with no `=` is also dropped. Values are trimmed and capped at 256 bytes; a value
may contain `=`, but a comma has no escape mechanism, so one embedded in a value
doesn't just split the pair — it can *manufacture* an extra tag
(`note=a,b=c` sends `note=a` **plus** a real `b=c`, and that phantom `b` key
consumes one of your 8 slots). At most 8 distinct keys per request; once the
limit is reached, further *new* keys are dropped (repeating an existing key
still overwrites it). The tags land on `traffic_event.details.clientTags`
(absent when no valid pair survives), so an external system within your domain
can read your own context back out of the traffic stream. Like the end-user and
session tags, client tags are caller-asserted, never validated, never resolved
against Nexus identities, and never used for routing / quota / hooks / caching /
IAM / metrics.

**Send opaque identifiers only, never personal data.** Client tags are stored
verbatim and are not covered by the compliance redaction pipeline (it governs
captured request/response bodies, not `details`). `details` is forwarded whole
to the operator's SIEM export and rendered in the control-plane traffic drawer,
and — unlike the account and identity fields — it is **not cleared by a GDPR
Article 17 erasure**. Anything you put in a tag value is queryable, exported,
and permanent; treat it the same way you'd treat `X-Nexus-End-User-Id`, but
across up to 8 keys instead of one.

The full header registry is [nexus-headers.md](../../cross-cutting/foundation/nexus-headers.md).
...
`error.type` carries the standard OpenAI vocabulary (`invalid_request_error`, `authentication_error`, `permission_error`, `not_found_error`, `rate_limit_error`, `api_error`). Upstream-originated errors map it from the normalised provider code in `internal/ingress/envelope/error_envelope.go`; gateway-generated errors derive it from the HTTP status in `openAIErrorTypeForStatus` (`proxy_errors.go`) — deriving from the status rather than a per-code table is deliberate, because the gateway emits ~45 distinct UPPER_SNAKE codes and a table would silently mis-default on every new one.

`error.code` carries the **Nexus** machine code in UPPER_SNAKE (`ROUTING_NO_MATCH`, `AUTH_INVALID_KEY`, `MODEL_REQUIRED`, `MODEL_NOT_ALLOWED`, `ENDPOINT_NOT_SUPPORTED`, `PROVIDER_UNAVAILABLE`) — the one deliberate divergence, see §8. `error.param` names the offending field where one is unambiguous (`model` for the model-resolution codes).

Endpoints the gateway does not serve return a JSON `ENDPOINT_NOT_SUPPORTED` envelope (`envelope.WriteEndpointNotSupported`, wired as the `/v1/` ServeMux fallback) rather than Go's default `text/plain` 404, so an SDK that JSON-parses every error body always finds a message.

All three behaviours landed in AP-3: before it, gateway-generated errors carried the constant `type: "proxy_error"` with no `param`, and an unmounted path answered `404 page not found` as text/plain — which the OpenAI SDKs surfaced as an error with an empty message.

## 8. Deliberate divergences from api.openai.com

The gateway diverges from stock OpenAI in the ways below, on purpose. Each row is pinned by a case in `tests/e2e-python/sdk_compat/test_divergence.py` (or the named module), so changing one has to break a test that explains why it exists. The reader-facing version of this table is [`docs/users/api/openai-sdk-compatibility.md`](../../../../users/api/openai-sdk-compatibility.md).

| Divergence | Behaviour | Rationale | Pinned by |
| --- | --- | --- | --- |
| `error.code` vocabulary | Nexus UPPER_SNAKE, not OpenAI's `model_not_found` / `invalid_api_key` | Matched by the Control Plane UI, the Hub alert aggregators, and the documented 429 discriminator. OpenAI's `error.code` is free-form, so SDK callers lose nothing — `error.type` carries the standard vocabulary. | `test_errors.py::test_error_code_stays_nexus_upper_snake` |
| Streamed `json_object` on a non-OpenAI target | Content may arrive markdown-fenced | The non-streaming path strips a stray fence (`unwrapJSONObjectFences`); doing it mid-stream would require buffering the whole completion, defeating streaming. | not pinned — documented limitation |
| Streaming `usage` injection | A terminal usage frame is emitted even when the caller did not set `stream_options.include_usage` | `ensureStreamUsage` (`specs/openai/codec/rewrite_native.go`) force-sets it upstream so cost is always metered, and nothing strips it on the way out. Surfaces as a chunk with empty `choices`. | `test_stream_usage_injected_without_opt_in` |
| `max_tokens` clamp | Values above the model's `maxOutputTokens` are lowered and disclosed in `X-Nexus-Coerced`; stock OpenAI 400s | Serving a smaller completion beats failing the call. | `test_max_tokens_clamped_to_model_ceiling_and_disclosed` |
| `max_tokens` rename | Renamed to `max_completion_tokens` for every `gpt-5*` family and the o-series | Those wires reject `max_tokens` (evidence on `NeedsMaxTokensRename`). Applies to `gpt-5.4` too, despite its sampling carve-out. | `test_max_tokens_renamed_on_gpt5_and_disclosed` |
| Sampling-param strip | `temperature` / `top_p` stripped for reasoning families, disclosed in `X-Nexus-Coerced` | Those wires 400 on caller-supplied sampling params. An unprobed new `gpt-5.x` fails safe (stripped, visible, no 400 risk). | `test_temperature_stripped_on_gpt5_and_disclosed` |
| `gpt-5.4` sampling carve-out | `gpt-5.4` keeps `temperature` | Probed 2026-07-16: accepts `temperature=0` on both wires. Pinned because the cheap "strip every `gpt-5*`" fix would silently degrade it. | `test_temperature_preserved_on_gpt54_carveout` |
| Cross-format param drop | `n`, `seed`, `logprobs`, `user`, `service_tier` dropped (not rejected) on non-OpenAI targets — `n=2` yields one choice with a 200 | No equivalent on the target wire; failing the request would be worse than serving it. | `test_cross_format_silently_drops_openai_only_params` |
| `reasoning_effort` drop | Dropped silently on non-OpenAI targets | As above. | `test_reasoning.py::test_reasoning_effort_dropped_cross_format` |
| Strict `json_schema` on Anthropic | Hard 400 naming `response_format`, canonical lowercase `invalid_request` | Anthropic has no schema-enforced mode. Degrading an output *contract* to a prompt hint is the silent-wrong-answer failure the codec exists to prevent — unlike `json_object`, where a prompt hint is a fair degradation. | `test_anthropic_target_rejects_json_schema_strict` |
...
- `packages/ai-gateway/internal/providers/specs/openai/rewrites/rewrites.go` — the per-model quirk table behind the §8 rename / strip / carve-out rows
- `tests/e2e-python/sdk_compat/` + `tests/e2e-node/` — the OpenAI SDK drop-in regression suites that pin §8

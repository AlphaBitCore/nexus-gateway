...
func TestRoundTripper_SwitchingProtocols_PreservesBody(t *testing.T) {
	// A 101 body must reach the caller untouched. loggingBody is io.ReadCloser
	// only, so wrapping it breaks coder/websocket's resp.Body.(io.ReadWriteCloser)
	// downcast and fails every WS dial with "response body is not a
	// io.ReadWriteCloser". The success record is emitted up front instead.

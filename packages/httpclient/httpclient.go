// Package httpclient is the single source of *http.Client construction for
// outbound calls across every service and test module in this repository.
//
// It is its own module, and deliberately so: its whole dependency closure is
// the standard library plus golang.org/x/net. Living inside packages/shared
// meant three modules could not use it — requiring shared would have pulled
// pgx, NATS and Redis into binaries that avoid them on purpose — and "that
// module cannot reach the factory" became the reason for a growing list of
// exceptions. A list of exceptions to a rule is evidence about the rule.
//
// Bare http.Client literals are forbidden, enforced by
// TestNoBareHTTPClientConstruction in this package: constructing one is a
// composite literal, which only the AST distinguishes from the type appearing
// in a parameter or a field. http.DefaultClient is forbidden by forbidigo,
// which can match that selector. Every outbound HTTP caller — provider
// adapters, webhook hooks, alerting, SIEM sinks, thingclient HTTP fallback —
// instantiates a tuned *http.Client through New or NewProbe at startup and
// reuses it for the lifetime of the process.
//
// Two Config fields exist for callers the tuned defaults do not fit, and both
// were added because their absence was being answered with an allowlist entry
// instead: NoTimeout, for a response with no meaningful total duration, and
// Transport, for a caller that must wrap, swap, adjust or replace the
// transport. Whatever Transport returns is still wrapped in this package's
// logging and request-id layer.
package httpclient
...
type Config struct {
	...
	// Zero means use 30s; see NoTimeout to ask for none.
	Timeout time.Duration

	// NoTimeout removes the client-level deadline entirely, for callers whose
	// response is a stream with no meaningful total duration — an SSE session,
	// a video artifact relay, a long-poll. It exists because a zero Timeout
	// cannot say this: zero is also what an unset field looks like, and that
	// one means 30s.
	//
	// The request context still bounds the call. A caller with neither is
	// asking for a goroutine that can hang forever.
	NoTimeout bool
	...
	// Transport, when non-nil, receives the tuned *http.Transport this
	// package builds and returns what the client should actually use. It is
	// the seam for the four things the tuned transport cannot express on its
	// own:
	//
	//   - wrapping it (a retry RoundTripper over the pool)
	//   - swapping it at runtime (an upstream whose config changes under load)
	//   - adjusting it (system roots before a pinned mTLS config exists)
	//   - ignoring it (an in-process dispatcher that never opens a socket)
	//
	// Whatever it returns is still wrapped in this package's logging and
	// request-id layer, so a caller reaching for this seam keeps the part of
	// the factory that is about observability rather than about pooling.
...
func New(cfg Config) *http.Client {
	...
	tr := &http.Transport{
		...
		// Stated rather than inherited. Go's client default has been TLS 1.2
		// since 1.18, so this changes no handshake — it puts the floor where
		// every client can be seen to have it, instead of in the one caller
	...
	return &http.Client{
		Timeout: cfg.Timeout,
		Transport: WrapTransport(rt, WrapOpts{
			Logger:         cfg.Logger,
			Caller:         cfg.Caller,

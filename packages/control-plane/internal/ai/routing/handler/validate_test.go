...
func TestValidateSmartRuleMatchConditions(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			raw:           `{}`,
			wantOK:        false,
			wantMsgSubstr: `must pin "requestedModelLiterals" to at least one request keyword`,
		},
		{
		...
		{
			name:         "smart with a keyword that is not auto accepted",
			strategyType: "smart",
			raw:          `{"requestedModelLiterals":["route-me"]}`,
			wantOK:       true,
		},
		{
			name:         "smart with several keywords accepted",
			strategyType: "smart",
			raw:          `{"requestedModelLiterals":["auto","fast","cheap"]}`,
			wantOK:       true,
		},
		{
			name:         "smart with a bounded glob accepted",
			strategyType: "smart",
			raw:          `{"requestedModelLiterals":["gpt-4-*"]}`,
			wantOK:       true,
		},
		{
			name:          "smart with the everything-glob rejected",
			strategyType:  "smart",
			raw:           `{"requestedModelLiterals":["*"]}`,
			wantOK:        false,
			wantMsgSubstr: `"*" matches every request`,
		},
		{
			name:          "smart with a repeated-star everything-glob rejected",
			strategyType:  "smart",
			raw:           `{"requestedModelLiterals":["**"]}`,
			wantOK:        false,
			wantMsgSubstr: `"**" matches every request`,
		...
		{
			name:          "smart with one everything-glob among real keywords rejected",
			strategyType:  "smart",
			raw:           `{"requestedModelLiterals":["auto","fast","*"]}`,
			wantOK:        false,
			wantMsgSubstr: `"*" matches every request`,
		},
		{
			name:          "smart with an empty keyword rejected",
			strategyType:  "smart",
			raw:           `{"requestedModelLiterals":[""]}`,
			wantOK:        false,
			wantMsgSubstr: `"" is not a request keyword`,
		},
		{
			name:          "smart with a blank keyword rejected",
			strategyType:  "smart",
			raw:           `{"requestedModelLiterals":["auto","   "]}`,
			wantOK:        false,
			wantMsgSubstr: `"   " is not a request keyword`,

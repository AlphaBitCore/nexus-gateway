...
// shadowResponse mirrors thingmgr.ShadowComparison so we can decode the
// per-key diff tree before flattening it.
...
// RenameShadowResponse flattens Hub's {keys:{k:{desired,reported,synced}}}
// shape into the UI's ConfigSyncComparison: top-level targetConfig / appliedConfig
// maps plus an explicit outOfSyncKeys list. outOfSync is derived from the
// per-key sync flags so the UI stays coherent even when Hub's version-only
// top-level Synced disagrees with per-key content comparisons.
func RenameShadowResponse(in []byte) ([]byte, error) {
	var sr shadowResponse
	if err := json.Unmarshal(in, &sr); err != nil {
		return nil, fmt.Errorf("hubadapter: unmarshal shadow: %w", err)
	}

	targetConfig := make(map[string]json.RawMessage, len(sr.Keys))
	appliedConfig := make(map[string]json.RawMessage, len(sr.Keys))
	outOfSyncKeys := make([]string, 0, len(sr.Keys))
	for k, diff := range sr.Keys {
		targetConfig[k] = diff.Desired
		appliedConfig[k] = diff.Reported
		if !diff.Synced {
			outOfSyncKeys = append(outOfSyncKeys, k)
		}
	}
	sort.Strings(outOfSyncKeys)

	return json.Marshal(map[string]any{
		"nodeId":         sr.ThingID,
		"targetConfig":   targetConfig,
		"appliedConfig":  appliedConfig,
		"targetVersion":  sr.DesiredVer,
		"appliedVersion": sr.ReportedVer,
		"outOfSync":      len(outOfSyncKeys) > 0,
		"outOfSyncKeys":  outOfSyncKeys,
	})
}

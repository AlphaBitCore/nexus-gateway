...
// TestRenameShadowResponse_FlattensKeys pins the contract against the real
// thingmgr.ShadowComparison shape: keys:{k:{desired,reported,synced}} must
// flatten into targetConfig / appliedConfig flat maps + an outOfSyncKeys
// list, with outOfSync derived from per-key sync flags (NOT top-level
// version-only synced, which can disagree with per-key content).
func TestRenameShadowResponse_FlattensKeys(t *testing.T) {
	in := []byte(`{
	  "thingId": "gw-1",
	  ...
	  "synced": true,
	  "keys": {
	    "routing_rules": {"desired": {"rules": []}, "reported": null, "synced": false},
	    "killswitch":    {"desired": {"engaged": false}, "reported": {"engaged": false}, "synced": true}
	  }
	}`)
	out, err := RenameShadowResponse(in)
	if err != nil {
		t.Fatalf("rename: %v", err)
	}
	...
	if got["targetVersion"] != float64(5) || got["appliedVersion"] != float64(5) {
		t.Errorf("version fields = %v / %v", got["targetVersion"], got["appliedVersion"])
	}

	target, ok := got["targetConfig"].(map[string]any)
	if !ok {
		t.Fatalf("targetConfig not a map: %v", got["targetConfig"])
	}
	applied, ok := got["appliedConfig"].(map[string]any)
	if !ok {
		t.Fatalf("appliedConfig not a map: %v", got["appliedConfig"])
	}
	if _, ok := target["routing_rules"]; !ok {
		t.Error("targetConfig missing routing_rules key")
	}
	if _, ok := applied["killswitch"]; !ok {
		t.Error("appliedConfig missing killswitch key")
	}

	// outOfSync must reflect per-key state, not the top-level version-only flag.
	if got["outOfSync"] != true {
		t.Errorf("outOfSync = %v; want true (one key desynced)", got["outOfSync"])
	}
	keys, ok := got["outOfSyncKeys"].([]any)
	if !ok {
		t.Fatalf("outOfSyncKeys not an array: %v", got["outOfSyncKeys"])
	}
	...
	sort.Strings(strs)
	if !reflect.DeepEqual(strs, []string{"routing_rules"}) {
		t.Errorf("outOfSyncKeys = %v; want [routing_rules]", strs)
	}

	// Internal terms must not leak.
	for _, forbidden := range []string{"thingId", "thingType", "desired", "reported", "desiredVer", "reportedVer", "keys", "synced"} {
		if _, bad := got[forbidden]; bad {
			t.Errorf("forbidden key %q leaked", forbidden)
		}
	}
}

// TestRenameShadowResponse_AllSynced covers the happy path where every key
// matches so outOfSyncKeys serializes as an empty array (NOT null) and
// outOfSync is false.
func TestRenameShadowResponse_AllSynced(t *testing.T) {
	in := []byte(`{
	  "thingId": "gw-1", "thingType": "ai-gateway",
	  "desiredVer": 1, "reportedVer": 1, "synced": true,
	  "keys": {"observability": {"desired": {"log": "info"}, "reported": {"log": "info"}, "synced": true}}
	}`)
	out, err := RenameShadowResponse(in)
	if err != nil {
		t.Fatalf("rename: %v", err)
	}
	if !containsJSON(out, `"outOfSync":false`) {
		t.Errorf("outOfSync should be false; got %s", out)
	}
	if !containsJSON(out, `"outOfSyncKeys":[]`) {
		t.Errorf("outOfSyncKeys should be []; got %s", out)
	}
}
...
func containsJSON(body []byte, substr string) bool {
	return string(body) != "" && bytesContains(body, []byte(substr))
}
...
func TestRename_UnmarshalErrorsAreWrapped(t *testing.T) {
	cases := []struct {
		...
		{"shadow", RenameShadowResponse, "hubadapter: unmarshal shadow", []byte(`not json`)},

...
// EnableDiagMode opens (or replaces) a diag-mode window and sets
// thing.metadata.diagModeUntil in a single transaction. Returns ErrThingNotFound
// (a sentinel exposed below) if the thing does not exist.
func (store *Store) EnableDiagMode(ctx context.Context, p EnableDiagModeParams) (*DiagModeWindow, error) {
	...
	// Stamp thing.metadata.diagModeUntil so shadow consumers (the agent's
	// OnConfigChanged for staticInfo-adjacent metadata) pick it up.
	//
	// jsonb_set rejects scalar inputs ("cannot set path in scalar",
	// SQLSTATE 22023). A simple COALESCE on a SQL NULL is not enough — the
	// `metadata` column also legitimately holds the JSONB scalar `null`
	// for things that were registered before metadata was populated. The
	// CASE expression below normalises any non-object value (JSONB null,
	// string, number, array, true/false) to an empty object before the
	// jsonb_set call, so the diag-mode stamp succeeds uniformly.
	untilJSON, _ := json.Marshal(p.Until.UTC().Format(time.RFC3339))
	if _, err := tx.Exec(ctx, `
		UPDATE thing
		   SET metadata = jsonb_set(
		           CASE
		               ...
		               ELSE metadata
		           END,
		           '{diagModeUntil}', $1::jsonb, true
		       ),
		       updated_at = NOW()
		 WHERE id = $2
	`, string(untilJSON), p.ThingID); err != nil {
		return nil, fmt.Errorf("enable_diag_mode metadata: %w", err)
	}
...
// DisableDiagMode closes any active window for the thing and clears
// thing.metadata.diagModeUntil. Returns ErrNoActiveDiagMode when there is no
// open window (so the handler can 404).
func (store *Store) DisableDiagMode(ctx context.Context, thingID string) error {
	...
	`, thingID); err != nil {
		return fmt.Errorf("disable_diag_mode metadata: %w", err)
	}

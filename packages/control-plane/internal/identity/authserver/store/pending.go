...
type PendingAuthzEntry struct {
	...
	IdPID string
	// IdPNonce is a single-use nonce generated by the OIDC SSO-start handler,
	// sent to the external IdP as the `nonce` authorize parameter, and verified
...
// SetIdPNonce stamps the single-use IdP nonce on an existing pending entry,
// returning false if no live entry is found. Called by the OIDC SSO-start
// handler so the callback can verify the returned ID token's nonce claim.
func (s *PendingAuthzStore) SetIdPNonce(authctx, nonce string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	e, ok := s.data[authctx]
	if !ok {

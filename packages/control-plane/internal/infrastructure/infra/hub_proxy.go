...
func (h *Handler) RegisterNodeRoutes(g *echo.Group, iamMW func(action string) echo.MiddlewareFunc) {
	...
	g.GET("/nodes/:id/config-sync", h.NodesConfigSync, iamMW(iam.ResourceNode.Action(iam.VerbRead)))
...
func (h *Handler) NodesConfigSync(c echo.Context) error {
	return h.hubForward(c, http.MethodGet, "/api/hub/things/"+url.PathEscape(c.Param("id"))+"/shadow", hub.RenameShadowResponse)
}

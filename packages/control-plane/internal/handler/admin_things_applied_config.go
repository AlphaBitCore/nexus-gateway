...
// configuration and its version come from the Node's shadow (same source used
// by /nodes/:id/config-sync) so both tabs share one version semantic; template
// state is only a fallback for keys not present in the target yet.
//

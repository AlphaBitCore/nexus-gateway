...
//     body is larger (without a backend an oversize body would otherwise be
//     inlined whole, which is what MaxInlineBodyBytes is named after preventing)
...
func EmitBody(
	...
	// No spill backend configured → inline, but BOUNDED.
	//
	// This is the COMMON production shape rather than a rare one: every
	// *.config.yaml ships spill disabled, so an unbounded arm here keeps an
	// oversize body whole under a setting literally named MaxInlineBodyBytes. A
	// 10 MiB body stored inline under a 256 KiB "max inline" value is both a name
	// that lies and an unbounded memory path on the audit row and the MQ message —
	// the same failure the failed-Put arm below avoids, on the path that runs far
	// more often.
	//
	...
	// kept" instead of quietly holding something nobody sized for. Truncated is
	// what tells a reader the row is a prefix; an operator who needs whole bodies
	// configures a spill backend, which is exactly the setting this path is the
	// absence of.
	if store == nil {
	...
	if err != nil {
		// Fall back to inline rather than drop the row — but BOUNDED.
		//
		// Returning the whole body is the one shape the spill threshold exists to prevent:
		// a body large enough to spill, inlined into the audit row and the MQ message. A
		// 200 MiB payload that fails to spill would be published as a 200 MiB NATS message,
		// and NATS enforces max_payload — so the "don't drop the row" intent loses the row
		// anyway, loudly, somewhere else, after the memory has already been spent three
		// times over (buffer, marshal, wire).
		//

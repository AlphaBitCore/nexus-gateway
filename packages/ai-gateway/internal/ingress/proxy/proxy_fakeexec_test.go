...
func TestServeProxy_Fake_PassthroughNoReshape(t *testing.T) {
	...
	// Bridge present; reshape is invoked (chat-completions ingress
	// always triggers it) and returns the body unchanged.
	...
	// OpenAI ingress → OpenAI target. The ChatCompletions arm of the
	// reshape guard fires (ingress.WireShape == WireShapeOpenAIChat is
	// true), so reshape WILL be invoked — but the fake just echoes the
	// input.
	// The point of this test is to drive the response path with a
	// permissive bridge so the success arm completes.
	if w.Code != http.StatusOK {
	...
	if !reshapeInvoked {
		t.Errorf("reshape MUST be invoked for chat-completions ingress (the guard's branch)")
	}
}

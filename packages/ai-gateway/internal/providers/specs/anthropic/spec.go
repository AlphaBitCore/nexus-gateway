...
func NewSpec(log *slog.Logger) provcore.AdapterSpec {
	...
	return provcore.AdapterSpec{
		...
		// The native /v1/messages differential — stamping the resolved
		// ProviderModelID into the body-root `model` — is owned by the
		// codec's RewriteNative (an aliased catalog model would otherwise
		// reach Anthropic verbatim and 404). Sampling-param wire quirks stay
		// in the codec's cross-format leg (Rule 3) and remain transparent on
		// the native passthrough for now.
	}
}

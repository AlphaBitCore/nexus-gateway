...
type Record struct {
	// EventID is this audit row's own identity — traffic_event.id, and the
	// key the consumer's ON CONFLICT (id) DO NOTHING uses to make a NATS
	// redelivery idempotent. That contract holds only while the key
	// identifies ONE event, so it is minted here and never derived from
	// anything a caller controls: an X-Nexus-Request-Id reused across two
	// distinct calls (routine in retry code) would otherwise make the second
	// event look like a redelivery of the first and be dropped silently.
	//
	// Stamped once per record by eventID() and reused by every later
	// conversion — marshalRecordPlain deliberately rebuilds the message a
	// second time on a marker collision, and both encodings must carry the
	// same id or the fallback would insert a duplicate row.
	EventID string

	// RequestID is the request id: the caller's X-Nexus-Request-Id, or its
	// X-Request-Id alias when the canonical name is absent, or a UUID the
	// middleware minted when neither arrived. It is persisted to
	// traffic_event.external_request_id, echoed on the response, and is the
	// key that stitches the several rows one request produces as it crosses
	// services. Repeating is allowed by design — a caller may reuse it, and
	// the multi-hop and realtime cases share one by construction.
	RequestID string
	// ClientRequestID is the caller's X-Request-Id exactly as they sent it,
	// kept so the response can echo back the spelling they used. It is not a
	// second correlation channel: when it is the only spelling that arrived it
	// IS the request id, and RequestID already carries that value.
	...
	// X-Nexus-End-User-Id header and nothing else — the ingress protocol's
	// native user field is addressed to the provider and answers a different
	// question (see enduser.go for the full argument);
	...
	SessionID string
	// ClientTags is the caller-declared tag bag — opaque key/value facts the
	// calling product asserts about this request, persisted onto
	// traffic_event.details.clientTags. Same contract as EndUserID: sourced
	// from the X-Nexus-Client-Tags header only, never validated, never joined,
	// never used for routing / quota / hooks / caching / IAM. Nil when the
	// caller declared nothing, which omits the details key entirely.
	ClientTags map[string]string
	// TraceID is the CALLER's own W3C distributed-trace id, lifted from an
	// inbound traceparent so a traffic row joins back to their APM. Empty when
	// the caller sent no traceparent, which is the common case: the column
	// stays NULL rather than receiving the trace id our own tracer derived
	// from RequestID, because a field meaning "the caller's trace" must be
	// empty when there is no caller trace to point at.
	//
	// This is NOT the cross-service correlation key — that is RequestID.
	TraceID   string
	Timestamp time.Time
	Method    string
	Path      string
	...
	// CacheMarkerInjected counts the prompt-cache markers the codec actually
	// wrote into the body it sent. Not what the operator configured: a caller
	// that sent its own cache_control is forwarded untouched and reports 0.
	CacheMarkerInjected int
	...
	// Both zero when smart routing did not run or the router call failed —
	// the strategy fails open, and a failed call is not charged.
	RouterCostUsd    float64
	RouterProviderID string

	// InternalOpsBreakdown itemises the internal model calls this request made
	// on the user's behalf, one entry per call, and is persisted to the
	// traffic_event.internal_ops_breakdown JSONB column that has carried this
	// documented shape since 2026-05-21 without a producer ever writing it.
	//
	// It exists because the internal-ops cost columns store amounts with no
	// basis. router_cost_usd says what a router call cost but not what it was
	// billed on, so when those calls were charging cached prompt tokens at the
	// full input rate, nothing in our own data could show it — the discrepancy
	// was only ever visible against the vendor bill, and no historical row can
	// be recomputed now. Recording the token split alongside the amount makes
	// the next such drift checkable from the row itself.
	//
	// A slice rather than pre-marshalled JSON because the routing stage drains
	// it incrementally, once per routing result; recordToMessage marshals it.
	InternalOpsBreakdown []InternalOpsEntry

	// EmbeddingProviderID is the provider that served the embedding call whose
	// cost is in EmbeddingCostUsd. Sibling of EmbeddingModelID.

...
type JobIntervalConfig struct {
	...
	// DiagModeExpiry is the cadence for clearing the diagModeUntil flag from
	// thing.metadata when the corresponding diag window has ended. Defaults
	// to 1 minute.
	DiagModeExpiry time.Duration `yaml:"diagModeExpiry"`
...
func applyEnvOverrides(cfg *HubConfig) {
	...
	parseDurationEnv("NEXUS_HUB_SCHEDULER_DIAG_MODE_EXPIRY_INTERVAL", &cfg.Scheduler.Intervals.DiagModeExpiry)

...
const STATUS_OPTIONS = [
  'online', 'offline', 'enrolled', 'out-of-sync', 'revoked', 'drift',
] as const;
...
function statusVariant(status: string): 'success' | 'warning' | 'default' | 'danger' | 'info' {
  const map: Record<string, 'success' | 'warning' | 'default' | 'danger' | 'info'> = {
    online: 'success',
    enrolled: 'warning',
    offline: 'default',
    'out-of-sync': 'danger',
...
export default function InfraNodesPage() {
  ...
  const columns: DataTableColumn<Node>[] = [
    ...
    { key: 'status', label: t('pages:infrastructure.colStatus'), render: (r) => <Badge variant={statusVariant(r.status)}>{r.status}</Badge> },

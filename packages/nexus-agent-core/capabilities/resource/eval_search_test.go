...
// raise them when retrieval improves, never lower them without explicit
...
var searchGoldenSet = []goldenQuestion{
	...
	// NOTE: the "rotate a device certificate" → rotateAgentCert question is
	// absent because that endpoint does not exist. The P-256 mTLS
	// surface — SignCSR / renew-cert / RotateAgentCert — is not exposed; the agent
...
// Baseline floors measured on the current implementation. CI fails if
// retrieval drops below these; raise after verified improvements.
//
// The embedded CP catalog excludes dead/withdrawn surface — ModelPricing
// endpoints + pricing.yaml, AdminAuditLog client* correlation fields,
// redaction_spans, applicableEndpoints, getEffective, and the P-256
// device-cert-rotation operations (rotateAgentCert etc.). The golden set tracks
// the catalog, so "rotate a device certificate" → rotateAgentCert is absent
// (the endpoint does not exist). The 30-question set measures to:
//
//	structural 100 / 100 / 100 (n=9)   summary 90 / 100 / 100 (n=11)
//	purpose     50 /  60 /  80 (n=10)  — the "purpose" misses (probeCredential /
//	providerTest, getMePermissions) are a known scorer limit: those endpoints
//	exist with intact descriptions, but intent-only phrasing does not retrieve
//	them in the top-K.
const (

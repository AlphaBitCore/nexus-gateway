...
func (p *ProxyServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if p.checker != nil {
		...
		if err := p.checker.CheckConnect(r.Context(), ip, host, port); err != nil {
			...
			// gates (IP allowlist, private IP, SNI mismatch) still reject —
			// they are security checks, not allowlist misses, and must
			// remain enforced even in this mode.
...
func categorizeAccessError(err error) string {
	...
	case errors.Is(err, access.ErrSNIMismatch):
		return "rejected_sni"
	default:

...
func TestInitCompliance_ReturnsPopulatedBundle(t *testing.T) {
	...
	if bundle.InterceptHandler == nil {
		t.Error("InterceptHandler should not be nil")
	}
...
func testOpsReg() *opsmetrics.Registry {
...
// bridge_inspect.go — InspectRequest / InspectResponse / NewUsageAccumulator / ExtractResponseUsage
// These methods delegate to b.InterceptHandler which requires a real
// agentcompliance.AgentPipeline + logger. We can construct minimal pipelines.

func TestBridge_InspectResponse_NilHandler(t *testing.T) {
	b := &ConnectionBridge{InterceptHandler: nil}
	result := b.InspectResponse(context.Background(), "host", "GET", "/", nil, nil)
	if result.Decision != "approve" {
		t.Errorf("nil handler: want approve, got %s", result.Decision)
	}
}

func TestBridge_NewUsageAccumulator_NilHandler(t *testing.T) {
	b := &ConnectionBridge{InterceptHandler: nil}
	acc := b.NewUsageAccumulator("openai", "gpt-4")
	if acc != nil {
		t.Error("nil handler: NewUsageAccumulator should return nil")
	}
}

func TestBridge_ExtractResponseUsage_NilHandler(t *testing.T) {
	b := &ConnectionBridge{InterceptHandler: nil}
	usage := b.ExtractResponseUsage(context.Background(), "host", "/path", nil, nil)
	if usage.Status != "non_llm" {
		t.Errorf("nil handler: want non_llm status, got %s", usage.Status)
	}
}
...
func TestBridge_InspectRequest_WithHandler(t *testing.T) {
	bundle := InitCompliance(ComplianceConfig{
		DefaultAction: "passthrough",
	}, discardLogger())
	b := &ConnectionBridge{
	...
	result := b.InspectRequest(context.Background(), "api.openai.com", "POST", "/v1/chat/completions", nil, []byte(`{"messages":[]}`))
	// Decision should be "approve" since no hook rules are loaded.
	if result.Decision != "approve" {
		t.Errorf("InspectRequest with no hooks: want approve, got %s", result.Decision)
	}
}

func TestBridge_InspectRequest_RejectHard(t *testing.T) {
	// Wire a real handler that has no hooks — result will be approve by default.
	// We can't easily inject a reject_hard hook without a full policy setup,
	// so we just verify the non-panic path and decision mapping.
	bundle := InitCompliance(ComplianceConfig{
		DefaultAction: "passthrough",
	}, discardLogger())
	b := &ConnectionBridge{
	...
	result := b.InspectRequest(context.Background(), "host", "GET", "/path", nil, nil)
	if result.Decision == "" {
		t.Error("InspectRequest should return non-empty decision")
	}
}

func TestBridge_InspectResponse_WithHandler(t *testing.T) {
	bundle := InitCompliance(ComplianceConfig{
		DefaultAction: "passthrough",
	}, discardLogger())
	b := &ConnectionBridge{
	...
	result := b.InspectResponse(context.Background(), "api.openai.com", "POST", "/v1/chat/completions", []byte(`{}`), nil)
	if result.Decision == "" {
		t.Error("InspectResponse with handler should return non-empty decision")
	}
}

func TestBridge_InspectResponse_NilBodyWithHandler(t *testing.T) {
	bundle := InitCompliance(ComplianceConfig{
		DefaultAction: "passthrough",
	}, discardLogger())
	b := &ConnectionBridge{
	...
	// nil body path (len(body)==0) skips ExtractResponseUsage.
	result := b.InspectResponse(context.Background(), "host", "GET", "/path", nil, nil)
	if result.Decision != "approve" {
		t.Errorf("nil body InspectResponse: want approve, got %s", result.Decision)
	}
}

func TestBridge_NewUsageAccumulator_WithHandler(t *testing.T) {
	bundle := InitCompliance(ComplianceConfig{
		DefaultAction: "passthrough",
	}, discardLogger())
	b := &ConnectionBridge{InterceptHandler: bundle.InterceptHandler}
	// NewUsageAccumulator may return nil for unknown providers — just ensure no panic.
	_ = b.NewUsageAccumulator("openai", "gpt-4o")
}

func TestBridge_ExtractResponseUsage_WithHandler(t *testing.T) {
	bundle := InitCompliance(ComplianceConfig{
		DefaultAction: "passthrough",
	}, discardLogger())
	b := &ConnectionBridge{InterceptHandler: bundle.InterceptHandler}
	usage := b.ExtractResponseUsage(context.Background(), "api.openai.com", "/v1/chat/completions", nil, []byte(`{}`))
	// Just verify no panic; status will be "non_llm" for unrecognized host.
	_ = usage
}

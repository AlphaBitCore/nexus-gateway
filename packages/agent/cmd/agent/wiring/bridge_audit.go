...
func (b *ConnectionBridge) OnFlowComplete(result api.FlowResult) {
	...
	// inside tlsbump for bumped inspect flows, on a separate path.
	//
	// The flow id doubles as the request id so compliance-proxy and ai-gateway
	// events generated by this flow join to it. TraceID stays empty: the tunnel
	// was never decrypted, so no traceparent was ever readable, and a field
	// meaning "the client's own trace" must not be filled with an id we minted.
	e := auditevent.Event{
		ID:                result.FlowID,
		ExternalRequestID: result.FlowID,

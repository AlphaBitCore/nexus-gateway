...
// list. The agent consumes 9 shadow keys plus one acknowledged-
// external no-op (diag_mode). A future edit that silently drops
// a known key would degrade the agent back to "ignore unknown" and Hub
...
func TestBuildConfigLoader_AllKeysRegistered(t *testing.T) {
	...
	want := []string{
		"agent_settings",
		"diag_mode", // acknowledged-external no-op
		"exemptions",
		"hooks",

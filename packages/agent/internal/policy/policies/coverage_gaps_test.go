...
func TestParseDiagMode_FlatUntilShape(t *testing.T) {
	// Admin-emitted shape: flat "diagModeUntil" string on agent_settings root.
	acc := accessorWithRaw("agent_settings", []byte(`{"diagModeUntil":"2026-06-01T12:00:00Z"}`))
...
func TestParseDiagMode_NestedActiveTrueWithoutUntil(t *testing.T) {
	// Nested shape where only Active=true is set (until="" ⇒ still considered on).
	acc := accessorWithRaw("agent_settings", []byte(`{"diagMode":{"active":true}}`))
	got := Build(acc, nil).DiagMode
	if got == nil || !got.Active || got.Until != "" {
		t.Errorf("nested active-only: got %+v", got)
	}
}

func TestParseDiagMode_NeitherShapePresentReturnsNil(t *testing.T) {
	// Payload parses cleanly but carries no diag-mode signal at all.
	acc := accessorWithRaw("agent_settings", []byte(`{"unrelated":42}`))
	got := Build(acc, nil).DiagMode
	if got != nil {
		t.Errorf("no diag signal should yield nil, got %+v", got)
	}
}

func TestParseDiagMode_MalformedJSONReturnsNil(t *testing.T) {
	acc := accessorWithRaw("agent_settings", []byte(`{"diagMode":`))
	got := Build(acc, nil).DiagMode
	if got != nil {

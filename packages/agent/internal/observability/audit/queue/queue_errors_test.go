...
func TestQueue_ClosedDBSurfacesErrors(t *testing.T) {
	...
	if err := q.SaveConfigSnapshot(1, "{}"); err == nil {
		t.Error("SaveConfigSnapshot on closed db must error")
	}
	if _, _, err := q.LoadLatestConfigSnapshot(); err == nil {
		t.Error("LoadLatestConfigSnapshot on closed db must error")
	}

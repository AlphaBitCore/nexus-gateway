...
// SaveConfigSnapshot + LoadLatestConfigSnapshot

func TestConfigSnapshot_SaveLoadRoundTrip(t *testing.T) {
	q, _ := newTempQueue(t)
	if err := q.SaveConfigSnapshot(1, `{"a":1}`); err != nil {
		t.Fatalf("save v1: %v", err)
	}
	if err := q.SaveConfigSnapshot(5, `{"a":5}`); err != nil {
		t.Fatalf("save v5: %v", err)
	}
	if err := q.SaveConfigSnapshot(3, `{"a":3}`); err != nil {
		t.Fatalf("save v3: %v", err)
	}
	v, data, err := q.LoadLatestConfigSnapshot()
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	if v != 5 {
		t.Errorf("version: got %d, want 5 (highest)", v)
	}
	if data != `{"a":5}` {
		t.Errorf("data: got %q, want %q", data, `{"a":5}`)
	}
}

func TestLoadLatestConfigSnapshot_EmptyReturnsErrNoRows(t *testing.T) {
	q, _ := newTempQueue(t)
	v, d, err := q.LoadLatestConfigSnapshot()
	if err == nil {
		t.Fatalf("expected ErrNoRows-class error on empty table")
	}
	if v != 0 || d != "" {
		t.Errorf("on error, version/data should be zero values; got (%d,%q)", v, d)
	}
}

func TestSaveConfigSnapshot_UpsertSameVersionReplacesData(t *testing.T) {
	// INSERT OR REPLACE semantics: re-saving the same version replaces
	// data without erroring (contract: caller pushes the latest blob
	// for the configured version).
	q, _ := newTempQueue(t)
	if err := q.SaveConfigSnapshot(7, "first"); err != nil {
		t.Fatalf("first: %v", err)
	}
	if err := q.SaveConfigSnapshot(7, "second"); err != nil {
		t.Fatalf("second: %v", err)
	}
	_, data, err := q.LoadLatestConfigSnapshot()
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	if data != "second" {
		t.Errorf("data: got %q, want %q (REPLACE semantics)", data, "second")
	}
}

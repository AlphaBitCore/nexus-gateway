...
// multi-statement transactions (MarkSynced, SaveConfigSnapshot, RecordLocal,
// LoadLatestConfigSnapshot) to prevent interleaving within a transaction.
type Queue struct {
	db *sql.DB
	mu sync.Mutex
}
...
func NewQueue(dbPath string, encryptionKey []byte) (*Queue, error) {
	...
	if _, err := db.ExecContext(context.Background(), `
		...
		-- Config snapshots persisted for offline fallback.
...
// --- V2: Config snapshot persistence ---

// SaveConfigSnapshot persists a config snapshot for offline fallback.
func (q *Queue) SaveConfigSnapshot(version int, data string) error {
	q.mu.Lock()
	defer q.mu.Unlock()
	_, err := q.db.ExecContext(context.Background(),
		`INSERT OR REPLACE INTO config_snapshots (version, data, fetched_at)
		 VALUES (?, ?, datetime('now'))`,
		version, data,
	)
	return err
...
// LoadLatestConfigSnapshot returns the most recent config snapshot, or nil if none.
func (q *Queue) LoadLatestConfigSnapshot() (version int, data string, err error) {
	q.mu.Lock()
	defer q.mu.Unlock()
	err = q.db.QueryRowContext(context.Background(),
		`SELECT version, data FROM config_snapshots ORDER BY version DESC LIMIT 1`,
	).Scan(&version, &data)
	if err != nil {
		return 0, "", err
	}

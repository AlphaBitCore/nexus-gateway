...
// (matching shared/configsync's ConfigSnapshot.HookConfigs envelope and the
// agent's AgentPipeline.ApplyHooksShadowState parser). Older builds and
// tests use "hooks". Accept either to stay forward-compatible during
// rollouts where the menu-bar app may be one version ahead of Hub.

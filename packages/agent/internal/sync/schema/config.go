...
type AgentConfig struct {
	...
	// InterceptMode is retained as a no-op yaml field so older agent.yaml
	// files that still set `interceptMode: ne` continue to parse. The
	// experimental pf alternative (E74) was retired before shipping;
	// every macOS build now uses NETransparentProxyProvider
	// unconditionally and no consumer reads this value. Safe to remove
	// once the on-disk yamls in the wild are confirmed cleaned.
	InterceptMode string `yaml:"interceptMode"`
...
func MergeConfig(local *AgentConfig, remote map[string]any) *AgentConfig {
	...
	if v, ok := remote["interceptMode"].(string); ok && v == "ne" {
		// Only "ne" is accepted now (pf path retired pre-ship). The
		// field is kept on the struct so older agent.yaml files that
		// still set it continue to parse without warning, but no
		// consumer reads the value — every macOS build uses NE
		// unconditionally.
		merged.InterceptMode = v
	}
